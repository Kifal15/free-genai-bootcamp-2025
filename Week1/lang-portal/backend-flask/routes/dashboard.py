...
def load(app):
    ...
    def get_study_stats():
        try:
            ...
            cursor.execute('''
                WITH word_stats AS (
                    SELECT word_id, COUNT(*) as total_attempts,
                    SUM ( CASE WHEN correct = 1 THEN 1 ELSE 0 END / COUNT(*) as sucess_rate
                    FROM word_review_items wri JOIN study_sessions ss ON wri.study_session_id = ss.id 
                           GROUP BY word_id 
            ...
            cursor.execute('''
                SELECT COUNT(DISTINCT group_id) as active_group 
                FROM study_sessions 
