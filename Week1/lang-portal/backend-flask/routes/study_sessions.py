  ...
  def post_study_sessions():  
    try: 
      ...
      if not group_id or not study_activity_id:
        return jsonify({"error":"no data recieved sorry"}, 404)   

      cursor = app.db.cursor()
      from datetime import datetime

      created_at = datetime.now()
      cursor.execute('INSERT INTO study_sessions  (group_id, study_activity_id, created_at) VALUES ? ? ? ',(group_id, study_activity_id , created_at))
      session_id = cursor.lastrowid
      cursor = app.db.commit()
